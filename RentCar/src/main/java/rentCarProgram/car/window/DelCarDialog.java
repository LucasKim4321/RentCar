package rentCarProgram.car.window;
...
public class DelCarDialog extends JDialog {
	...
	JTextField tfSearch, tfCarNumber, tfCarcarName, tfCarColor, tfDisplacement, tfManufacturer,tfSegment;
	JButton btnSearch, btnDelete, btnCancel;

	// 생성자	
	public DelCarDialog(CarController carController, String str) {
	...
	class MemberBtnHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnSearch) {  // 조회 버튼 동작
				...
				// 회원 정보 DB수정 요청
				int result = carController.removeCar(vo);  // 애초에 carNumber가 서버에서 primary 설정이라 중복이면 안만들어지긴 함
				System.out.println("result: "+result);  // 만들기 성공하면 1 실패하면 0
				
				if (result > 0) {
					showMessage("차량을 삭제했습니다.",result);
				
				} else {
					showMessage("차량 삭제 실패",result);
				}
